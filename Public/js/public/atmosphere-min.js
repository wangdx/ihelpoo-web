...
B();
ak()
}function ak(){if(M.id){clearTimeout(M.id)
}if(C!=null){C.close();
C=null
}if(E!=null){E.abort();
E=null
...
}if(aA.trackMessageLength){aB=ax.partialMessage;
var az=[];
...
var aD=false;
var aI=false;
if(a.util.browser.opera&&ax.transport==="streaming"&&ax.readyState>2&&ay.readyState===4){ak();
aA();
return
...
if((!ax.enableProtocol||!az.firstMessage)&&ax.transport!=="polling"&&ay.readyState===2){am(ax)
}if(aI){var aC=0;
...
if(aA.connectTimeout>-1){aA.id=setTimeout(function(){if(aA.requestCount===0){ak();
...
}if(ay.connectTimeout>-1){ay.id=setTimeout(function(){if(ay.requestCount===0){ak();
