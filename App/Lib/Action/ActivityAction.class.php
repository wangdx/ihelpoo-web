...
class ActivityAction extends Action {
	...
	public function item()
    {
    	...
    	if (!empty($_GET['action'])) {
    		...
    		if ($getAction == "selectsure") {
    			$recordActivityUser = $ActivityUser->where("aid = $activityid AND uid = $userloginid")->find();
    			if (empty($recordActivityUser['id'])) {
    				redirect('/activity/item/'.$activityid, 3, '需要先加入此次活动, 才能选择Partner:) 3秒后页面跳转...');
    			}
    			if ($recordActivityUser['invite_status'] == 2) {
    				redirect('/activity/item/'.$activityid, 3, '你已经有了Partner了哦 :) 3秒后页面跳转...');
    			}
    			if ($recordActivityUser['invite_status'] == 3) {
    				redirect('/activity/item/'.$activityid, 3, '你已经选择了一个Partner 对方拒绝后才能选择第二个 :) 3秒后页面跳转...');
    			}
    			...
    			if (!empty($parteruid)) {
    				...
    				/**
    				 * update ActivityUser invite_status
    				 * 1 for had received invite @partner
    				 * 2 for partner ok @partner
    				 * 3 for invited other @self
    				 */
    				$recordActivityUserSelf = $ActivityUser->where("aid = $activityid AND uid = $userloginid")->find();
    				$updateActivityUserInvitestatus = array(
    					'id' => $recordActivityUserSelf['id'],
    					'invite_status' => 3,
    				);
    				$ActivityUser->save($updateActivityUserInvitestatus);
    				...
    				redirect('/activity/parterinvite/'.$activityid.'?invite=true', 3, '成功选择Partner 等待对方确认 :) 3秒后页面跳转...');
    			}
    		}
    		...
    		if ($getAction == "selectrandomsure") {
    			...
    			$ActivityUserinvite = M("ActivityUserinvite");
    			$searchRandUserSql = "SELECT * FROM i_activity_user WHERE aid = '$activityid' AND uid != '$userloginid' AND invite_status != '2' ORDER BY RAND() LIMIT 1";
    ...
    public function parterinvite()
    {
    			...
    			if (!empty($resultsActivityUserinvite)) {
    				foreach ($resultsActivityUserinvite as $resultActivityUserinvite) {
    					if ($resultActivityUserinvite['invite_uid'] == $inviteuserid || $resultActivityUserinvite['invite_uid'] == $userloginid) {
	    					...
	    					if ($resultActivityUser['invite_status'] != 2) {
    	...
    	if (!empty($isParterActivityUsers)) {
	    	$UserLogin = M("UserLogin");
    ...
    public function lotterydraw()
    {
    	...
    	if ($this->isPost()) {
    		...
    		if (is_int($lotterydrawActivityUser)) {
    			$userids = "<a href='".__ROOT__."/wo/".$recordsActivityUser[$lotterydrawActivityUser]['uid']."' class='getuserinfo' userid='".$recordsActivityUser[$lotterydrawActivityUser]['uid']."'>".$recordsActivityUser[$lotterydrawActivityUser]['nickname']."</a>";
    		} else {
    			$userids = NULL;
