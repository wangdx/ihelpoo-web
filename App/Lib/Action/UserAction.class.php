    ...
    public function register()
    {
        ...
        if ($this->isPost()) {
	        ...
	        if (!$result) {
	             ...
	             * iuc user data
	             */
	            $url = "http://ihelpoousercenter.sinaapp.com/iuc/returnautoincrementuid?email=".urlencode($email)."&password=".urlencode($password)."&nickname=".urlencode($nickname)."&sex=".$sex."&enteryear=".$enteryear."&school=hbmy&pw=".md5('ihelpoo2013');
        		$userdatacontents = file_get_contents($url);
        		if ($userdatacontents != 'false') {
        			$userdatacontentArray = json_decode($userdatacontents,TRUE);
        			if (is_array($userdatacontentArray)) {
        ...
        if ($this->isPost()) {
        	...
        	if ($result) {
        		...
        		if (!empty($userExist['uid'])) {
        			...
        			/**
        			 * iuc user data
        			 * move user data from ihelpoousercenter
        			 */
        			$url = "http://ihelpoousercenter.sinaapp.com/iuc/emailpulldata?useremail=".urlencode($email)."&pw=".md5('ihelpoo2013');
        			$userdatacontents = file_get_contents($url);
        			if ($userdatacontents != 'false') {
        				$userdatacontentArray = json_decode($userdatacontents,TRUE);
        				if ($userdatacontentArray['school'] == 'zzuli') {
        					$userdatacontentArraySchool = '郑州轻工业学院';
        				} else {
        					$userdatacontentArraySchool = '其他学校';
        				}
			             ...
			             * update i_user_login
			        	*/
        				$newUserloginData = array(
        					'uid' => $userdatacontentArray['uid'],
			            	'status' => $userdatacontentArray['status'],
							'email' => $userdatacontentArray['email'],
			            	'password' => $userdatacontentArray['password'],
			            	'nickname' => $userdatacontentArray['nickname'],
			            	'sex' => $userdatacontentArray['sex'],
			            	...
			            	'icon_fl' => 1,
        					'icon_url' => $newImageName,
        					'school' => $userdatacontentArray['school']
        				);
        				$IUserLogin->add($newUserloginData);
        				...
        				$newUserInfoData = array(
        					...
        					'mobile' => $userdatacontentArray['mobile'],
			            	'qq' => $userdatacontentArray['qq'],
			            	'weibo' => $userdatacontentArray['weibo'],
			            	'dynamic' => 1,
				            'fans' => 0,
				            'follow' => 0
        				);
			             ...
			             * user login weibo
			             */
			            if (!empty($userdatacontentArray['weibo_uid'])) {
				            $UserLoginWb = M("UserLoginWb");
				            $dataUserLoginWb = array(
			            	    'uid' => $userdatacontentArray['uid'],
			                    'weibo_uid' => $userdatacontentArray['weibo_uid'],
			            		'switch' => 1,
				            );
				            $UserLoginWb->add($dataUserLoginWb);
			            }
			             ...
			             */
			            $MsgSystem = M("MsgSystem");
			            $msgRegisterContent = "欢迎".$userdatacontentArray['nickname']."从".$userdatacontentArraySchool."来到民院我帮圈圈:) 这里一切都是新的开始!";
			            $msgRegisterData = array(
		                	'id' => NULL,
		                	'uid' => $userdatacontentArray['uid'],
		                 	'type' => 'system',
			             ...
			             * add default dynamic record.
			             */
			            $recordDynamicContent = "我刚刚从".$userdatacontentArraySchool."串校加入了民院我帮圈圈:)";
			            $RecordSay = M("RecordSay");
			            $RecordDynamic = M("RecordDynamic");
			            $newRecordSayData = array(
			            	'uid' => $userdatacontentArray['uid'],
			            ...
			            Vendor('Ihelpoo.Email');
		                $emailObj = new Email();
			            $emailObj->welcome($userdatacontentArray['email'], $userdatacontentArray['nickname']);
        				
        				$status = "movedata";
        				$info = "您的邮箱已经注册过我帮圈圈".$userdatacontentArraySchool."站，数据迁移成功，可以直接登录了 :)";
        			}
        		}
        	} else {
