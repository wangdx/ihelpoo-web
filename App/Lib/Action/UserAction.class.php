    ...
    public function loginweibo()
    {
        ...
        if (!empty($_POST['i_w_user_id'])) {
            ...
            if (empty($isWeiboExist['uid'])) {
                ...
                $postSendContent = '系统为您自动分配了我帮圈圈登录账号:'.$email.' 密码为:'.$password.'。为方便保证独立账号登录，希望您能及时重新设置账号和密码:)';
    ...
    public function register()
    {
        ...
        if (!empty($_POST['getschoollist'])) {
        	...
        	foreach ($resultsSchoolInfo as $schoolInfo) {
        		echo "<li><span class='gray'>[".strtoupper($schoolInfo['initial'])."]</span> <a href='".__ROOT__."/user/register?school=$schoolInfo[id]'>$schoolInfo[school]</a></li>";
        	}
        	echo '</ul>';
        	echo '<br /><p><a href="/about/sns" class="f12 gray fr">还没你们学校?</a></p><br />';
        	echo '</div>';
        	exit();
        }
        ...
        if ($this->isPost()) {
	        ...
	        if (!$result) {
	            ...
	            if (empty($nickname)) {
	            	redirect('/user/register/'.$userId, 3, '昵称不能全是特殊字符哦，3秒后跳转...');
	            }
